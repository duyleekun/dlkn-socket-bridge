/**
 * Cloudflare Worker entry point for zalo-worker.
 *
 * Routes:
...
import { onCallback, onSocketClosed } from "./adapter/on-callback";
import {
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "POST" && url.pathname.startsWith("/cb/")) {
      ...
      try {
        ...
        const json = JSON.parse(text) as { event?: string; reason?: string; code?: number };
        if (json.event === "closed") {
          await onSocketClosed(env, url.origin, sessionKey, json);
          return new Response("ok");
        }
      } catch {
